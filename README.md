...
| ✅ 6 | Двухпанельный Viewer (Reference + Edit, независимые вкладки уровня/языка). Обложка 1024×1536 (портрет). Автор на обложке с обводкой. Стоимость ~$0.04 у кнопки Generate. Автозаполнение Title при выборе 1 рассказа из коллекции. Кнопка Review для каждого рассказа коллекции. Исправлен баг двойного сохранения в вьюере (переключение уровня/языка перезаписывало контент). Для каждого рассказа коллекции — кнопки «🖼️ Cover ~$0.04» (авто-генерация с названием рассказа) и «📎 Upload» (своё изображение). Viewer показывает название рассказа вместо «(untitled)» для однограздных коротких историй. |
| 🔜 7 | Ридер: отображение chapter_N.jpg перед главами. Batch API (−50% стоимость). GitHub autopush. |
